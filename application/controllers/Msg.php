...
class Msg extends CI_Controller
{
    ...
    public function msgs() // Msg/msgs
    {
        ...
        $data['title'] = translate('Unread Messages');
    ...
    {
        // $data['all_properties_msgs'] = $this->Msg_m->get();
        $data['title'] = translate('Read Messages');
...
';*/
           ...
           <a  title="الإطلاع علي تفاصيل الرسالة " href="#modal_details" data-toggle="modal" onclick="get_msg_details(' . $item->msg_id . ')" 
           class="btn btn-warning btn-sm"> <i class="fa fa-eye"></i></a>
   
           
                    <a href="#" title="' . translate('Mark the message as read') . '"  onclick=\'Swal.fire({
                                            title: "' . translate('Are you sure to mark the message as read?') . '",
                                            text: "",
                                            icon: "warning",
                                            ...
                                            Swal.fire("' . translate('Mark the message as read') . '", "", "success");
    ...
    function get_ajax_msgs_read()
    {
      ...
      ';*/
            $row[] = '<a class="text-danger"><i class="far fa-calendar mr-1"></i>' . $item->act_ended_date . '</a>
              ...
              ';
            $row[] = '<a  title="'.translate('Show The Details').'" href="#modal_details" data-toggle="modal" onclick="get_msg_details(' . $item->msg_id . ')" class="btn btn-warning btn-sm"> <i class="fa fa-eye"></i></a>
   ';
            $data[] = $row;
        }
        $output = array(
...
/*            $this->session->set_flashdata('success', 'تم تحديد الرسالة كرسالة مقروئة');*/
            $this->session->set_flashdata('success',translate('Marked the message as read') );
