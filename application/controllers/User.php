...
class User extends CI_Controller
{
    ...
    public function adduser()
    {
        ...
        $this->form_validation->set_rules('passconf', translate('Password Confirmation'), 'required|matches[password]',
...
//$emailContect = "Your Password Is : $randomDigits";
    ...
    public function edit($id)
    {
        ...
        if ($this->input->post('password')) {
            ...
            $this->form_validation->set_rules('passconf', translate('Password Confirmation'), 'matches[password]',
                array('matches' => 'You must provide a %s.')
            );
        }
        if ($this->input->post('passconf')) {
    ...
    public function check_user_data()
    {
        if ($this->input->post('user_phone')) {
            ...
            if (isset($data) && !empty($data) && ($data > 0)) {
                $response = array('valid' => false, 'message' => translate('The mobile number is already registered'));
        ...
        if ($this->input->post('username')) {
            ...
            if (isset($data) && !empty($data) && ($data > 0)) {
                $response = array('valid' => false, 'message' => translate('Customer name already registered'));
                echo json_encode($response);
                return;
            } else {
                $response = array('valid' => true);
