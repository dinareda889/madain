...
//            echo '<br>'. $datafile['file_name'];
    ...
    public function edit_video($id)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $this->session->set_flashdata('danger', translate('error try again'));
            }
            redirect('Video_c', 'refresh');
        }
    }
    public function delete_video($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('success', translate('Delete Done Successfully'));
        }
        redirect('Video_c', 'refresh');
    }
    ...
    public function check_youtube($url){
        $parts = parse_url($url);
        $this->form_validation->set_message('check_youtube', translate('Should be youtube link'));
